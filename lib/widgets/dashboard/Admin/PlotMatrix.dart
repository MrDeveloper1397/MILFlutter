        ...
        value.forEach((val) {
          if (selectedPlots!.ventureName.toString() == val.ventureName) {
            if (val.sectorCd.toString().trim() == sectorTxt) {
              ...
              if (allotedCount == 0) {
                allotedCount = val.status == 'Y' ? val.total! : 0;
                allotedExtend = val.status == 'Y' ? val.extend! : '0';
                allotedStatus = 'Y';
                fetchStatus = allotedStatus;
              }
              if (availableCount == 0) {
                availableCount = val.status == 'N' ? val.total! : 0;
                availableExtend = val.status == 'N' ? val.extend! : '0';
                availableStatus = 'N';
              ...
              if (mortgageCount == 0) {
                mortgageCount = val.status == 'M' ? val.total! : 0;
                mortgageCountExtend = val.status == 'M' ? val.extend! : '0';
                mortgageStatus = 'M';
                fetchStatus = mortgageStatus;
              }
              if (registeredCount == 0) {
                registeredCount = val.status == 'G' ? val.total! : 0;
                registeredCountExtend = val.status == 'G' ? val.extend! : '0';
                registeredStatus = 'G';
